...
if __name__ == '__main__':
    ...
    all_gll_r = np.sqrt((all_gll_x - par['REF_X0'])**2 +
                        (all_gll_y - par['REF_Y0'])**2)

    all_gll_theta = (np.arctan2(all_gll_y - par['REF_Y0'], all_gll_x -
    ...
    if par['INTEGRATE']:
        ...
        Y = R * np.sin(T) + par['REF_Y0']

        # interpolate noise mask into the polar grid
        points = np.zeros((ngll, 2))
        points[:, 0] = np.squeeze(all_gll_x)
        points[:, 1] = np.squeeze(all_gll_y)

        mask_noise_intp = griddata(points, all_mask_noise, (X, Y),
    ...
    if par['WRITE_FILES']:
        ...
        for p in range(0, par['NPROC']):
            ...
            normal_x_noise = np.ones(mask_noise.shape) * par['XDIR']
            normal_y_noise = np.ones(mask_noise.shape) * par['YDIR']
            normal_z_noise = np.ones(mask_noise.shape) * par['ZDIR']
    ...
    if par['PLOT_MASK']:
        ...
        fig, ax = plt.subplots()
        im = ax.scatter(all_gll_x, all_gll_y, c=all_mask_noise_ring)
        ax.set_xlabel('x [m]')
        ax.set_ylabel('y [m]')
